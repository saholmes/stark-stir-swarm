...
pub fn make_domain_2048() -> (F, usize) {
    ...
    // Use u64: TWO_ADICITY = 32 on Goldilocks / Pallas, which overflows
    // usize on wasm32 (32-bit usize).
    let max_n: u64 = 1u64 << t;
    let max_root = F::get_root_of_unity(max_n)
        .expect("Field should provide a 2^t root of unity for its two-adicity");
    // Downscale to order N: omega = max_root^(2^t / N)
