...
mod tests {
	...
	/// GATE prove-S2-dbladd-O (O-aware assembled double-and-add ROUND) — extends the assembled
	/// round to correctly handle the POINT AT INFINITY O (the Jacobian point with Z=0, canonical
	/// (1,1,0)). A complete constant-time scalar-mul accumulator STARTS at O and passes THROUGH it
	/// in the leading rounds, so a full round must compute
	///     A' = (A==O) ? (b ? P : O) : (b ? [2]A+P : [2]A).
	/// This welds the existing 53-table generic round (b?[2]A+P:[2]A, whose final selector now
	/// EXPORTS its result over internal channels ch_gen_* instead of the output boundaries) to
	/// three O-aware pieces:
	///   (1) SOUND Z1==0 flag. A committed bit `z_is_zero` is FORCED to equal (Z1==0) by two gated
	///       directions living in the output table: (A) if the flag is 0 then a seamed ModMul
	///       Z1·Zinv (Zinv committed once in a source table, pushed to the flag's fe_inv gate) must
	///       reduce to 1 — pinned by `(1+flag)·(prod+1)==0`, so a Z1=0 prover cannot produce a
	///       residue of 1 and the flag is forced to 1; (B) if the flag is 1 then every bit of the
	///       channel-bound Z1 must be 0 — `flag·Z1==0`, so a Z1≠0 prover cannot claim O and the
	///       flag is forced to 0. Together flag = (Z1==0), unforgeable.
	///   (2) O branch `b ? P : O` computed in-circuit as a per-coordinate GF(2) mux of P against the
	///       constant O=(1,1,0): o_x = 1 + b·(1+Px), o_y = 1 + b·(1+Py), o_z = b·Pz (the round's
	///       conditional-add selector applied with the O coordinates as the false-branch constants).
	///   (3) OUTPUT mux A' = z_is_zero ? o_branch : generic, per coordinate `ap = gen + flag·(gen+o)`,
	///       exposed on the output boundaries. When A=O the generic branch produces meaningless
	///       values, but its witness is populated with the RAW add-2007-bl/dbl formula outputs on the
	///       actual (X1,Y1,0) inputs (pure mul/add/sub, well-defined for Z=0 — NOT the native
	///       jac_dbl/jac_add O-shortcuts), so every generic constraint still holds; the result is
	///       simply discarded by the output mux.
	/// Total 53 (generic) + 1 (mm_zinv fe_inv) + 1 (Zinv source) + 1 (O-branch + flag + output mux)
	/// = 56 tables, W=1024. All four cases (A=O b∈{0,1}, A≠O b∈{0,1}) VALIDATE and match the native
	/// `round_o` reference; the A=O,b=1⇒P case is full PROVEN+VERIFIED (it exercises the O-branch mux
	/// AND the generic branch's Z=0-satisfiability). A forged flag (claim O when Z1≠0, or ≠O when
	/// Z1=0) and a forged input coordinate are both REJECTED. Residual case (out of scope, negligible
	/// for random signature points): the u1==u2 / A=±P doubling-exception inside jac_add.
	#[test]
	fn ec_weierstrass_dbl_add_round_oaware_over_b256() {
		use crate::b256_field::{B256TowerFamily, B256 as OurB256, U256};
		use crate::nonnative::{ripple_add, shl, two_pow_w_minus, write_bit, write_col, Adder, ModMul, ModMulRow};
		use binius_core::constraint_system::channel::{ChannelId, FlushDirection};
		use binius_core::fiat_shamir::HasherChallenger;
		...
		let c_p_bits = two_pow_w_minus(&to_bits(&p));
		let c_p_arr: [B1; W] = std::array::from_fn(|i| if c_p_bits[i] { B1::ONE } else { B1::ZERO });
		// O-branch/flag constants: `ones` = all-ones (boolean NOT of the flag) and `oneval` = the
		// integer 1 (bit0 set) — both O.x/O.y and the fe_inv residue-1 pin reuse `oneval`.
		let ones_arr: [B1; W] = [B1::ONE; W];
		let oneval_arr: [B1; W] = std::array::from_fn(|i| if i == 0 { B1::ONE } else { B1::ZERO });

		// A=(ax1,ay1,az1): the Jacobian accumulator (the non-O case reuses the generic round's point).
		let ax1_base = BigUint::parse_bytes(b"5a6b7c8d9e0f10213243546576879a0b1c2d3e4f5061728394a5b6c7d8e9f001", 16).unwrap() % &p;
		let ay1_base = BigUint::parse_bytes(b"7f0e1d2c3b4a5968778695a4b3c2d1e0f00112233445566778899aabbccddee02", 16).unwrap() % &p;
		let az1_base = BigUint::parse_bytes(b"026d3e4a5b6c7d8e9fa0b1c2d3e4f5060718293a4b5c6d7e8f90a1b2c3d4e5f6", 16).unwrap() % &p;
		// P=(px,py,pz): the base point added on a set bit (fixed across all four cases).
		let px = BigUint::parse_bytes(b"11335577991bb3d5f7192a4c6e8090a1c3e5072941638507a9cbed0f21436587", 16).unwrap() % &p;
		let py = BigUint::parse_bytes(b"6e5d4c3b2a1908f7e6d5c4b3a29180716f5e4d3c2b1a0918273645362718f0e3", 16).unwrap() % &p;
		let pz = BigUint::parse_bytes(b"03fedcba98765432100123456789abcdef0f1e2d3c4b5a69788796a5b4c3d2e1", 16).unwrap() % &p;

		// Native O-aware round: A=O ⇒ b?P:O; else double, then conditionally add P.
		let round_o = |a: &Jac, b: bool, pt: &Jac| -> Jac {
			if is_zero(&a.2) {
				if b { pt.clone() } else { jac_id() }
			} else {
				let d = jac_dbl(a, &p);
				if b { jac_add(&d, pt, &p) } else { d }
			}
		};

		// a_is_o ⇒ A=O=(1,1,0); bit = the scalar bit b; forge: 0=none, 1=flag, 2=input coordinate.
		let run = |a_is_o: bool, bit: bool, forge: u8, full: bool| -> (bool, String, bool) {
			let allocator = Bump::new();
			let mut cs = ConstraintSystem::<OurB256>::new();

			// A for this case: O = (1,1,0) with Z=0, else the generic accumulator.
			let (ax1, ay1, az1) = if a_is_o {
				(BigUint::from(1u32), BigUint::from(1u32), zero_big())
			} else {
				(ax1_base.clone(), ay1_base.clone(), az1_base.clone())
			};

			// ── RAW double strand D=[2]A (add-2007-bl/dbl formulas, NO O-shortcut — well-defined at Z=0). ──
			let d_delta = (&az1 * &az1) % &p;
			let d_gamma = (&ay1 * &ay1) % &p;
			let d_beta = (&ax1 * &d_gamma) % &p;
			let d_xmd = ((&ax1 + &p) - &d_delta) % &p;
			...
			// ── RAW add strand T=D+P (add-2007-bl; point1=D, point2=P; z1=Dz, x1=Dx, y1=Dy). ──
			let a_z1z1 = (&d_z3 * &d_z3) % &p;
			let a_z2z2 = (&pz * &pz) % &p;
			let a_u1 = (&d_x3 * &a_z2z2) % &p;
			let a_u2 = (&px * &a_z1z1) % &p;
			...
			// For A≠O the raw strands must equal the native point ops (and dodge the u1==u2 exception).
			if !a_is_o {
				let d_pt = jac_dbl(&(ax1.clone(), ay1.clone(), az1.clone()), &p);
				assert_eq!((d_x3.clone(), d_y3.clone(), d_z3.clone()), d_pt, "double strand D != native jac_dbl");
				let t_pt = jac_add(&d_pt, &(px.clone(), py.clone(), pz.clone()), &p);
			...
			// Native reference output A' = round_o(A, b, P), the boundary the round is gated against.
			let out_pt = round_o(&(ax1.clone(), ay1.clone(), az1.clone()), bit, &(px.clone(), py.clone(), pz.clone()));
			...
			// fe_inv witness for the flag gate: Zinv=Z1⁻¹ (prod=Z1·Zinv=1) when Z1≠0; when Z1=0 no
			// inverse exists so Zinv=0 and prod=0 (which the flag's direction-A pin catches).
			let zinv = if a_is_o { zero_big() } else { fe_inv(&az1, &p) };
			let prod = (&az1 * &zinv) % &p; // 1 when Z1≠0, 0 when Z1=0
			let true_flag = is_zero(&az1);
			let flag_val = if forge == 1 { !true_flag } else { true_flag };

			// generic-branch selector output (raw) and O-branch output (P vs O=(1,1,0)) by bit b.
			let gen_x = if bit { a_x3.clone() } else { d_x3.clone() };
			let gen_y = if bit { a_y3.clone() } else { d_y3.clone() };
			let gen_z = if bit { a_z3.clone() } else { d_z3.clone() };
			let one = BigUint::from(1u32);
			let o_x = if bit { px.clone() } else { one.clone() };
			let o_y = if bit { py.clone() } else { one.clone() };
			let o_z = if bit { pz.clone() } else { zero_big() };
			// output-mux value under the WITNESS flag (== out_pt when the flag is honest).
			let ap_x = if flag_val { o_x.clone() } else { gen_x.clone() };
			let ap_y = if flag_val { o_y.clone() } else { gen_y.clone() };
			let ap_z = if flag_val { o_z.clone() } else { gen_z.clone() };
			...
			// ── D-export channels feeding the add gadget's first point + the generic mux ──
			let chdx = cs.add_channel("chDx");
			let chdy = cs.add_channel("chDy");
			let chdz = cs.add_channel("chDz");
			...
			// ── generic-branch selector output channels (fed into the O-aware output mux) ──
			let ch_gen_x = cs.add_channel("chGenX");
			let ch_gen_y = cs.add_channel("chGenY");
			let ch_gen_z = cs.add_channel("chGenZ");
			// ── O-detection channels: Zinv source → mm_zinv.b, mm_zinv.r → flag's residue-1 pin ──
			let ch_zinv = cs.add_channel("chZinv");
			let ch_zprod = cs.add_channel("chZprod");

			// ── A' output channels (output mux → output boundaries) ──
			let chxp = cs.add_channel("chXprime");
			let chyp = cs.add_channel("chYprime");
			let chzp = cs.add_channel("chZprime");
			...
			// fe_inv gate ModMul: prod = Z1·Zinv (mod p) — Z1 pulled from d_chz1 (the SAME injected
			// accumulator Z), Zinv from its source, prod pushed to the flag's residue-1 pin.
			let mm_zinv = ModMul::<W>::build_seamed_in2_chain(&mut cs, &p_bits, np, d_chz1, ch_zinv, ch_zprod);
			...
			// ── D-export fan-out: Dx ×2 (add-u1 + generic mux), Dy ×2 (add-t1 + mux), Dz ×5 (add ×4 + mux). ──
			let mut fdx = cs.add_table("export Dx");
			let (fdx_c, fdx_pull) = pull_word(&mut fdx, d_chx3, "fDx");
			let fdx_push = push_word(&mut fdx, chdx, fdx_c, "fDx", 2);
			let fdx_id = fdx.id();
			...
			// ── GENERIC conditional-add selector: gen = b ? T : D, per coordinate, out = d + bbc·(d+t). ──
			// (Identical to the base round's mux, but its result is EXPORTED over ch_gen_* — not the
			// output boundaries — so the O-aware output mux can override it when A=O.)
			let mut mux = cs.add_table("dbl-add MUX b?T:D");
			let (mux_dx, mux_dx_sel) = pull_word(&mut mux, chdx, "muxDx");
			let (mux_tx, mux_tx_sel) = pull_word(&mut mux, a_chx3, "muxTx");
			let (mux_dy, mux_dy_sel) = pull_word(&mut mux, chdy, "muxDy");
			...
			let mux_ox = mux.add_committed::<B1, W>("mux_ox");
			mux.assert_zero("mux_x", mux_ox - (mux_dx + mbbc * (mux_dx + mux_tx)));
			let mux_ox_push = push_word(&mut mux, ch_gen_x, mux_ox, "muxOx", 1);
			let mux_oy = mux.add_committed::<B1, W>("mux_oy");
			mux.assert_zero("mux_y", mux_oy - (mux_dy + mbbc * (mux_dy + mux_ty)));
			let mux_oy_push = push_word(&mut mux, ch_gen_y, mux_oy, "muxOy", 1);
			let mux_oz = mux.add_committed::<B1, W>("mux_oz");
			mux.assert_zero("mux_z", mux_oz - (mux_dz + mbbc * (mux_dz + mux_tz)));
			let mux_oz_push = push_word(&mut mux, ch_gen_z, mux_oz, "muxOz", 1);
			let mux_id = mux.id();

			// ── Zinv SOURCE: commit Z1⁻¹ once and push it to the fe_inv gate (mm_zinv.b). ──
			let mut zisrc = cs.add_table("Z1⁻¹ source (push ×1)");
			let zi_col = zisrc.add_committed::<B1, W>("o_zinv");
			let zi_sel: [Col<B1, 64>; 4] = std::array::from_fn(|i| zisrc.add_selected_block::<B1, W, 64>(format!("o_zinv_psel{i}"), zi_col, i));
			let zi_b64: [Col<B64, 1>; 4] = std::array::from_fn(|i| zisrc.add_packed::<B1, 64, B64, 1>(format!("o_zinv_pb64{i}"), zi_sel[i]));
			zisrc.push_with_opts(ch_zinv, zi_b64, FlushOpts { multiplicity: 1, selector: None });
			let zisrc_id = zisrc.id();

			// ── O-BRANCH + SOUND FLAG + OUTPUT MUX (one table): binds z_is_zero to (Z1==0) and uses it
			// to select A' = z_is_zero ? (b?P:O) : gen, per coordinate. ──
			let mut omux = cs.add_table("O-aware MUX + Z1==0 flag");
			// prod (Z1·Zinv) from the fe_inv gate, Z1 (re-pulled) for the all-zero direction, the
			// generic selector output, and P for the O-branch.
			let (o_prod, o_prod_sel) = pull_word(&mut omux, ch_zprod, "oProd");
			let (o_z1, o_z1_sel) = pull_word(&mut omux, d_chz1, "oZ1");
			let (o_genx, o_genx_sel) = pull_word(&mut omux, ch_gen_x, "oGenX");
			let (o_geny, o_geny_sel) = pull_word(&mut omux, ch_gen_y, "oGenY");
			let (o_genz, o_genz_sel) = pull_word(&mut omux, ch_gen_z, "oGenZ");
			let (o_px, o_px_sel) = pull_word(&mut omux, a_chx2, "oPx");
			let (o_py, o_py_sel) = pull_word(&mut omux, a_chy2, "oPy");
			let (o_pz, o_pz_sel) = pull_word(&mut omux, a_chz2, "oPz");
			// z_is_zero flag broadcast to W bits (kbc trick: CircularLeft self-eq bound to one bit).
			let o_flag = omux.add_committed::<B1, 1>("o_flag");
			let o_flag_bc = omux.add_committed::<B1, W>("o_flag_bc");
			let o_flag_bcr = omux.add_shifted("o_flag_bcr", o_flag_bc, WLOG, 1, ShiftVariant::CircularLeft);
			omux.assert_zero("o_flag_eq", o_flag_bc - o_flag_bcr);
			let o_flag_l0 = omux.add_selected("o_flag_l0", o_flag_bc, 0);
			omux.assert_zero("o_flag_bind", o_flag_l0 - o_flag);
			// O-branch bit b broadcast (the round bit; only the ACTIVE branch's bit affects A').
			let o_b = omux.add_committed::<B1, 1>("o_b");
			let o_b_bc = omux.add_committed::<B1, W>("o_b_bc");
			let o_b_bcr = omux.add_shifted("o_b_bcr", o_b_bc, WLOG, 1, ShiftVariant::CircularLeft);
			omux.assert_zero("o_b_eq", o_b_bc - o_b_bcr);
			let o_b_l0 = omux.add_selected("o_b_l0", o_b_bc, 0);
			omux.assert_zero("o_b_bind", o_b_l0 - o_b);
			// constants: `ones` = per-lane NOT(flag); `oneval` = the integer 1 (O.x/O.y and residue pin).
			let o_ones = omux.add_constant("o_ones", ones_arr);
			let o_oneval = omux.add_constant("o_oneval", oneval_arr);
			// FLAG SOUNDNESS — direction A: flag=0 ⇒ prod==1  →  (1+flag)·(prod+1)==0 (a Z1=0 prover
			// yields prod=0≠1 so it cannot claim flag=0); direction B: flag=1 ⇒ Z1==0 → flag·Z1==0 (a
			// Z1≠0 prover cannot zero the channel-bound Z1 so it cannot claim flag=1). ⇒ flag=(Z1==0).
			omux.assert_zero("o_dirA", (o_ones + o_flag_bc) * (o_prod + o_oneval));
			omux.assert_zero("o_dirB", o_flag_bc * o_z1);
			// O-branch b?P:O with O=(1,1,0): o_x = 1 + b·(1+Px), o_y = 1 + b·(1+Py), o_z = b·Pz.
			let o_ox = omux.add_committed::<B1, W>("o_ox");
			omux.assert_zero("o_ox_eq", o_ox - (o_oneval + o_b_bc * (o_oneval + o_px)));
			let o_oy = omux.add_committed::<B1, W>("o_oy");
			omux.assert_zero("o_oy_eq", o_oy - (o_oneval + o_b_bc * (o_oneval + o_py)));
			let o_oz = omux.add_committed::<B1, W>("o_oz");
			omux.assert_zero("o_oz_eq", o_oz - o_b_bc * o_pz);
			// OUTPUT mux A' = flag ? o_branch : gen, per coordinate ap = gen + flag·(gen+o); expose A'.
			let o_apx = omux.add_committed::<B1, W>("o_apx");
			omux.assert_zero("o_apx_eq", o_apx - (o_genx + o_flag_bc * (o_genx + o_ox)));
			let o_apx_push = push_word(&mut omux, chxp, o_apx, "oApX", 1);
			let o_apy = omux.add_committed::<B1, W>("o_apy");
			omux.assert_zero("o_apy_eq", o_apy - (o_geny + o_flag_bc * (o_geny + o_oy)));
			let o_apy_push = push_word(&mut omux, chyp, o_apy, "oApY", 1);
			let o_apz = omux.add_committed::<B1, W>("o_apz");
			omux.assert_zero("o_apz_eq", o_apz - (o_genz + o_flag_bc * (o_genz + o_oz)));
			let o_apz_push = push_word(&mut omux, chzp, o_apz, "oApZ", 1);
			let omux_id = omux.id();

			// forge==2 ⇒ corrupt the accumulator's X1 boundary (witness stays honest ⇒ channel imbalance).
			let ax1_pub = if forge == 2 { (&ax1 + 1u32) % &p } else { ax1.clone() };
			let boundaries = vec![
				// accumulator A in: X1 ×2 (mm_beta + gpm), Y1 ×3 (mm_gamma×2 + gyz), Z1 ×5 (mm_delta×2 +
				// gyz + mm_zinv.a + omux flag-pull).
				Boundary { values: to_boundary(&ax1_pub), channel_id: d_chx1, direction: FlushDirection::Push, multiplicity: 2 },
				Boundary { values: to_boundary(&ay1), channel_id: d_chy1, direction: FlushDirection::Push, multiplicity: 3 },
				Boundary { values: to_boundary(&az1), channel_id: d_chz1, direction: FlushDirection::Push, multiplicity: 5 },
				// base point P in: X2 ×2 (mm_u2 + omux), Y2 ×2 (mm_t2 + omux), Z2 ×5 (mm_z2z2×2 + mm_t1 +
				// g3 + omux).
				Boundary { values: to_boundary(&px), channel_id: a_chx2, direction: FlushDirection::Push, multiplicity: 2 },
				Boundary { values: to_boundary(&py), channel_id: a_chy2, direction: FlushDirection::Push, multiplicity: 2 },
				Boundary { values: to_boundary(&pz), channel_id: a_chz2, direction: FlushDirection::Push, multiplicity: 5 },
				// A' out (= native round_o)
				Boundary { values: to_boundary(&out_pt.0), channel_id: chxp, direction: FlushDirection::Pull, multiplicity: 1 },
				Boundary { values: to_boundary(&out_pt.1), channel_id: chyp, direction: FlushDirection::Pull, multiplicity: 1 },
				Boundary { values: to_boundary(&out_pt.2), channel_id: chzp, direction: FlushDirection::Pull, multiplicity: 1 },
			];
			// 53 (generic round) + 1 (mm_zinv) + 1 (Zinv source) + 1 (O-branch/flag/output mux) = 56.
			let statement = Statement { boundaries, table_sizes: vec![1; 56] };
			let mut witness = WitnessIndex::<OurB256>::new(&cs, &allocator);
			let fill = |seg: &mut TableWitnessSegment<OurB256>, sel: &[Col<B1, 64>; 4], bits: &[bool]| {
				for (i, &s) in sel.iter().enumerate() {
					write_col::<64>(seg, s, 0, &bits[i * 64..i * 64 + 64]).unwrap();
			...
			fill_mm(&mut witness, &mm_z1z2sq, &a_z1z2, &a_z1z2, &a_z1z2_sq);
			fill_mm(&mut witness, &mm_z3, &a_zt, &a_h, &a_z3);
			// fe_inv gate ModMul: Z1·Zinv = prod (1 when Z1≠0, 0 when Z1=0).
			fill_mm(&mut witness, &mm_zinv, &az1, &zinv, &prod);
			...
			// GENERIC MUX witness (gen = bit ? T : D, raw)
			{
				let tw = witness.init_table(mux_id, 1).unwrap();
				let mut seg = tw.full_segment();
				write_col::<W>(&mut seg, mux_dx, 0, &to_bits(&d_x3)).unwrap();
				...
				write_col::<W>(&mut seg, mbbcr, 0, &bb).unwrap();
				write_bit(&mut seg, mbl0, 0, bit).unwrap();
				write_col::<W>(&mut seg, mux_ox, 0, &to_bits(&gen_x)).unwrap();
				fill(&mut seg, &mux_ox_push, &to_bits(&gen_x));
				write_col::<W>(&mut seg, mux_oy, 0, &to_bits(&gen_y)).unwrap();
				fill(&mut seg, &mux_oy_push, &to_bits(&gen_y));
				write_col::<W>(&mut seg, mux_oz, 0, &to_bits(&gen_z)).unwrap();
				fill(&mut seg, &mux_oz_push, &to_bits(&gen_z));
			}

			// Zinv source witness
			{
				let tw = witness.init_table(zisrc_id, 1).unwrap();
				let mut seg = tw.full_segment();
				let zib = to_bits(&zinv);
				write_col::<W>(&mut seg, zi_col, 0, &zib).unwrap();
				fill(&mut seg, &zi_sel, &zib);
			}

			// O-branch + flag + output mux witness
			{
				let tw = witness.init_table(omux_id, 1).unwrap();
				let mut seg = tw.full_segment();
				let prodb = to_bits(&prod);
				write_col::<W>(&mut seg, o_prod, 0, &prodb).unwrap();
				fill(&mut seg, &o_prod_sel, &prodb);
				let z1b = to_bits(&az1);
				write_col::<W>(&mut seg, o_z1, 0, &z1b).unwrap();
				fill(&mut seg, &o_z1_sel, &z1b);
				write_col::<W>(&mut seg, o_genx, 0, &to_bits(&gen_x)).unwrap();
				fill(&mut seg, &o_genx_sel, &to_bits(&gen_x));
				write_col::<W>(&mut seg, o_geny, 0, &to_bits(&gen_y)).unwrap();
				fill(&mut seg, &o_geny_sel, &to_bits(&gen_y));
				write_col::<W>(&mut seg, o_genz, 0, &to_bits(&gen_z)).unwrap();
				fill(&mut seg, &o_genz_sel, &to_bits(&gen_z));
				write_col::<W>(&mut seg, o_px, 0, &to_bits(&px)).unwrap();
				fill(&mut seg, &o_px_sel, &to_bits(&px));
				write_col::<W>(&mut seg, o_py, 0, &to_bits(&py)).unwrap();
				fill(&mut seg, &o_py_sel, &to_bits(&py));
				write_col::<W>(&mut seg, o_pz, 0, &to_bits(&pz)).unwrap();
				fill(&mut seg, &o_pz_sel, &to_bits(&pz));
				// flag broadcast
				write_bit(&mut seg, o_flag, 0, flag_val).unwrap();
				let fb = vec![flag_val; W];
				write_col::<W>(&mut seg, o_flag_bc, 0, &fb).unwrap();
				write_col::<W>(&mut seg, o_flag_bcr, 0, &fb).unwrap();
				write_bit(&mut seg, o_flag_l0, 0, flag_val).unwrap();
				// O-branch bit broadcast (= round bit b)
				write_bit(&mut seg, o_b, 0, bit).unwrap();
				let obb = vec![bit; W];
				write_col::<W>(&mut seg, o_b_bc, 0, &obb).unwrap();
				write_col::<W>(&mut seg, o_b_bcr, 0, &obb).unwrap();
				write_bit(&mut seg, o_b_l0, 0, bit).unwrap();
				// constants
				...
				// O-branch coordinates b?P:O and the output-muxed A'
				write_col::<W>(&mut seg, o_ox, 0, &to_bits(&o_x)).unwrap();
				write_col::<W>(&mut seg, o_oy, 0, &to_bits(&o_y)).unwrap();
				write_col::<W>(&mut seg, o_oz, 0, &to_bits(&o_z)).unwrap();
				write_col::<W>(&mut seg, o_apx, 0, &to_bits(&ap_x)).unwrap();
				fill(&mut seg, &o_apx_push, &to_bits(&ap_x));
				write_col::<W>(&mut seg, o_apy, 0, &to_bits(&ap_y)).unwrap();
				fill(&mut seg, &o_apy_push, &to_bits(&ap_y));
				write_col::<W>(&mut seg, o_apz, 0, &to_bits(&ap_z)).unwrap();
				fill(&mut seg, &o_apz_push, &to_bits(&ap_z));
			}
		...
		// validate-only: all four honest cases (A=O and A≠O × b∈{0,1}).
		let (v_oo0, e_oo0, _) = run(true, false, 0, false);
		assert!(v_oo0, "honest A=O, b=0 (A'=O) failed validate_witness: {e_oo0}");
		let (v_oo1, e_oo1, _) = run(true, true, 0, false);
		assert!(v_oo1, "honest A=O, b=1 (A'=P) failed validate_witness: {e_oo1}");
		let (v_g0, e_g0, _) = run(false, false, 0, false);
		assert!(v_g0, "honest A≠O, b=0 (A'=[2]A) failed validate_witness: {e_g0}");
		let (v_g1, e_g1, _) = run(false, true, 0, false);
		assert!(v_g1, "honest A≠O, b=1 (A'=[2]A+P) failed validate_witness: {e_g1}");

		// soundness: a forged flag (claim the WRONG A==O status) is REJECTED, in BOTH directions.
		let (vf_o, _efo, _) = run(true, true, 1, false); // Z1=0 but flag claims ≠O ⇒ residue-1 pin fails
		assert!(!vf_o, "SOUNDNESS FAILURE: forged flag (Z1=0 claimed as ≠O) was accepted");
		let (vf_g, _efg, _) = run(false, true, 1, false); // Z1≠0 but flag claims =O ⇒ all-zero pin fails
		assert!(!vf_g, "SOUNDNESS FAILURE: forged flag (Z1≠0 claimed as O) was accepted");
		// soundness: a forged input coordinate is REJECTED (channel imbalance).
		let (vc, _ec, _) = run(false, true, 2, false);
		assert!(!vc, "SOUNDNESS FAILURE: a forged accumulator coordinate was accepted in the O-aware round");

		// one full prove+verify: A=O, b=1 ⇒ A'=P (exercises the O-branch mux AND the generic branch's
		// Z=0-satisfiability).
		let t0 = std::time::Instant::now();
		let (vok, verr, verify_ok) = run(true, true, 0, true);
		let elapsed = t0.elapsed();
		assert!(vok, "honest O-aware round (full) failed validate_witness: {verr}");
		assert!(verify_ok, "assembled O-aware P-256 double-and-add round must PROVE+VERIFY over B256");

		println!(
			"GATE prove-S2-dbladd-O: O-AWARE P-256 double-and-add ROUND A'=(A==O)?(b?P:O):(b?[2]A+P:[2]A) PROVEN+VERIFIED over B256 @L1(128) in {elapsed:?}; the 53-table generic round exports its selector output over ch_gen_*, a seamed fe_inv ModMul (Z1·Zinv) + Zinv source table detect O via a SOUND z_is_zero flag (direction A: flag=0⇒prod==1; direction B: flag=1⇒Z1==0 ⇒ flag=(Z1==0)), and an output mux table computes the O-branch (b?P:O over O=(1,1,0)) and selects A'=flag?o_branch:gen — 56 tables total. All four cases (A=O/A≠O × b∈{{0,1}}) match native round_o; the A=O,b=1⇒P case was full-proven; a forged flag (both directions) and a forged input coordinate are REJECTED. The point-at-infinity-aware atomic step of a complete constant-time in-circuit ECDSA scalar multiplication. RESIDUAL (out of scope): the u1==u2 / A=±P doubling-exception in jac_add (negligible for random signature points)."
		);
	}
