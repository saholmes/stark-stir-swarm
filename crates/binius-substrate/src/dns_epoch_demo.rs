...
pub fn run_dns_epoch_demo(zone: &[DnsRecord], per_record_width: usize, level: Sha3Level) -> Result<DemoReport> {
	...
	//     reach the category. L1/L3 run over B256 (κ_IT ≤ 192); L5 runs over the **B512 tower**
	//     (`measure_epoch_verify_b512_hash`, GF(2^512) per-record muls) so κ_IT = 256 — no cap.
	...
	let m = match level {
		...
		Sha3Level::L5 => measure_epoch_verify_b512_hash::<Sha3_512, Sha3Compression<Sha3_512>>(per_record_width, &[n], 256)?,
	};
