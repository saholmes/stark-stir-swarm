...
mod tests {
	...
	/// GATE prove-S3-1b (S3 full PKCS#1-v1.5 verify, bit-vector path) — a GENUINE RSA signature
	/// verifies end-to-end over B256, with the recovered encoded message bound to the FULL EMSA-
	/// PKCS1-v1.5 structure (not just the digest). A real RSA-496 keypair with e=3 (generated by the
	/// `rsa` crate) signs M; the signature s satisfies s³ mod N = EMSA-PKCS1-v1.5(SHA-256(M), 62). The
	/// circuit proves the modexp s³ mod N as two seam-glued ModMul strands (s² then s²·s) at W=1024,
	/// and PINS the final output boundary to the expected EMSA encoding as ceil(np/64)=8 B64 lanes —
	/// binding ALL 496 bits, i.e. the 0x00 01 ‖ FF…FF ‖ 00 ‖ DigestInfo ‖ SHA-256(M) padding in full,
	/// not merely the low 256. Balanced iff s³ mod N equals the exact EMSA encoding of M under this
	/// key ⇒ ACCEPT. A signature checked against a DIFFERENT message pins the boundary to that
	/// message's EMSA, which s³ does not equal, so the boundary unbalances ⇒ REJECT — a signature does
	/// not verify for the wrong message. e=3 (two multiplies) keeps it tractable; real RSA-2048/65537
	...
	fn rsa_pkcs1_verify_emsa_bound_prove_over_b256() {
		...
		// Genuine RSA-496 keypair with e = 3, seeded for reproducibility.
		let mut rng = StdRng::seed_from_u64(0xB1_2560_5341);
		...
		let n_lanes = (np + 63) / 64; // 8 for a 496-bit modulus — binds the whole EMSA encoding
		let n_bits = to_bits(&n);

		let msg = b"STARK-Binius S3 prove-S3-1: genuine RSA-496 PKCS1-v1.5";
		let digest = sha256(msg);
		let sig_bytes = sk.sign(Pkcs1v15Sign::new::<Sha256>(), &digest).expect("sign");
		let s = BigUint::from_bytes_be(&sig_bytes);
		let s2 = (&s * &s) % &n;
		let em = (&s2 * &s) % &n; // recovered encoded message = s³ mod N

		// Native RSA verify: em is the EMSA-PKCS1-v1.5 encoding of SHA-256(M) (full structure).
		let k = (np + 7) / 8; // RSA octet length = 62 for a 496-bit modulus
		let expected = emsa_pkcs1_sha256(&digest, k);
		let em_expected = BigUint::from_bytes_be(&expected);
		assert_eq!(em, em_expected, "genuine RSA-496 sig must recover the EMSA encoding of M");
		assert!(rsa_pkcs1_sha256_verify_k(&n, 3, msg, &s, k), "native RSA-496 verify must accept");

		// The EMSA encoding of a DIFFERENT message — s³ does NOT equal this, so it must REJECT.
		let em_other = {
			let d2 = sha256(b"a different message");
			let enc = emsa_pkcs1_sha256(&d2, k);
			BigUint::from_bytes_be(&enc)
		};
		...
		// One proof: s pushed (mult 3: squared twice + multiplied once), em pinned to the EMSA encoding.
		let run = |em_pub: &BigUint, full: bool| -> (bool, String, bool) {
			...
			// s² (pull s twice, push s²); em = s²·s (pull s², pull s, push em).
		...
		let (vok, verr, verify_ok) = run(&em, true);
		assert!(vok, "honest RSA-496 verify failed validate_witness: {verr}");
		assert!(verify_ok, "genuine RSA-496 PKCS1-v1.5 signature must PROVE+VERIFY over B256");

		let (v2, _e, _) = run(&em_other, false);
		assert!(!v2, "SOUNDNESS FAILURE: the signature verified against the WRONG message's EMSA");

		println!(
			"GATE prove-S3-1b: genuine RSA-496 PKCS#1-v1.5 signature VERIFIES over B256 @L1(128) — s³ mod N proven as two seam-glued ModMul strands (W=1024), final output boundary pinned to the FULL EMSA-PKCS1-v1.5(SHA-256(M)) encoding via ceil(np/64)=8 B64 lanes (binds all 496 bits: 00 01 FF..FF 00 DigestInfo ‖ H); ACCEPT for M, REJECT against a different message's EMSA. Real RSA-2048 = same construction at W=4096."
		);
	}
