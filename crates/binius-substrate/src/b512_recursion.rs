...
// onto the 512-bit challenge/extension field `B512TowerFamily` (tower level 9), so
// the seam, root-boundary and cross-table channel-join all prove AND verify at NIST
// L5 Fiat–Shamir security (security_bits = 256) with their adversarial rejects
// firing.
//
// This mirrors `b256_recursion.rs` exactly, ONE tower level up (B256 -> B512). These
// are the B128 primitives in `sha3_seam.rs` (M2b-2 + M2b-3) and `sha3_join.rs`
// (M2b-4) re-threaded through the B512 prove wiring proven out in `b512_keccak.rs`.
// NO FORK CHANGE is required: every m3 op these primitives use — `add_shifted`,
// `add_selected_block`, `add_packed`, `push`, `pull`, `assert_zero`, `add_constant`
// — lives on `impl<'a, F: TowerField> TableBuilder<'a, F>` and is generic over the
// top field; `Statement<F>` and `Boundary<F>` are likewise generic. The ONLY thing
// that changes vs the B256 versions is:
//   * `ConstraintSystem::<B512>` / `TableBuilder<'_, B512>` / `WitnessIndex::<B512>`,
//     so the witness segment is `TableWitnessSegment<B512>`;
//   * `prove/verify::<U512, B512TowerFamily, Sha256, Sha256Compression,
//     HasherChallenger<Sha256>>`;
//   * the public boundary values are elements of the TOP field, which is now `B512`
//     (the `FExt` slot), so a claimed root lane is `B512::from(B64::new(lane))`
//     instead of `B256::from(..)`. This is the single spot where the concrete top
//     field is named; it is in OUR harness (not a fork op), and the fix is
//     mechanical because `Boundary<F>` is generic.
//
...
// field parameter.
//
// SOUNDNESS BOUNDARY: identical to the B128/B256 M2b-2/3/4 (see those modules).
// Moving to B512 only raises the challenge/extension field to 2^512 so the
// FRI/sumcheck error terms clear NIST L5(256); it changes nothing about WHICH
// relations are in-circuit. In particular M2b-4 remains aggregation / intra-proof
// cross-table binding, NOT proof-carrying recursion (the parent does not verify the
// child's STARK in-circuit). We do NOT overclaim.
...
/// Witness segment packed type over B512 (B512 is its own width-1 packed field).
type Seg<'a> = binius_m3::builder::TableWitnessSegment<'a, OurB512>;
/// Table builder over the B512 top field.
type Tbl<'a> = TableBuilder<'a, OurB512>;

/// Write a track-0 constant `val` (interior tracks 0) into every row of a
/// `Col<B1,512>` witness buffer over the B512 segment.
...
/// tuple form pushed to / pulled from a channel. Over the B512 top field.
...
/// Convert a 32-byte claimed root into 4 top-field (`B512`) boundary values: each
/// 8-byte little-endian lane is a `B64` embedded into `B512` (the channel operates
/// in the top field). This is the B512 analogue of the B256 `B256::from(B64::new)` —
/// the ONE spot the concrete top field is named.
fn claimed_root_to_boundary_values(claimed_root: [u8; 32]) -> Vec<OurB512> {
	(0..4)
		.map(|i| {
			let lane = u64::from_le_bytes(claimed_root[i * 8..i * 8 + 8].try_into().unwrap());
			OurB512::from(B64::new(lane))
		})
		.collect()
}
...
/// over the B512 top field.
pub struct SeamTableB512 {
...
impl SeamTableB512 {
	pub fn new(cs: &mut ConstraintSystem<OurB512>) -> Self {
		Self::build(cs, None)
	}

	pub(crate) fn new_with_root_boundary(
		cs: &mut ConstraintSystem<OurB512>,
		root_channel: ChannelId,
	) -> Self {
		Self::build(cs, Some(root_channel))
	}

	fn build(cs: &mut ConstraintSystem<OurB512>, root_channel: Option<ChannelId>) -> Self {
		let mut table = cs.add_table("SHA3-256 depth-2 binding seam over B512 (M2b-2/3)");
...
/// Honest end-to-end depth-2 seam over B512. Returns `(proof_size, g1_digests, roots)`.
pub fn prove_verify_seam_b512(
	children: &[(([u8; 32], [u8; 32]), [u8; 32])],
	log_inv_rate: usize,
	security_bits: usize,
	...
	let table = SeamTableB512::new(&mut cs);
	...
	binius_core::constraint_system::validate::validate_witness(&ccs, &[], &witness)
		.map_err(|e| anyhow::anyhow!("honest seam witness failed validate_witness over B512: {e}"))?;
...
/// Public wrapper: forged g1->g2 link over B512 (the M2b-2 deliverable).
pub fn forged_link_rejected_b512(
	forged_prefix: [u8; 32],
	log_inv_rate: usize,
	security_bits: usize,
) -> Result<SeamRejectReport> {
...
// ============================================================================
// M2b-3 — expose the depth-2 chain ROOT as a PUBLIC BOUNDARY over B512.
// ============================================================================
...
/// M2b-3 over B512. Build the honest depth-2 seam for a SINGLE chain, PUSH the g2
/// digest to a channel, and supply `claimed_root` as the channel's public PULL in
/// `Statement.boundaries` (top-field = B512).
pub fn prove_verify_seam_root_boundary_b512(
	children: &[(([u8; 32], [u8; 32]), [u8; 32])],
	claimed_root: [u8; 32],
	log_inv_rate: usize,
	security_bits: usize,
	...
	let table = SeamTableB512::new_with_root_boundary(&mut cs, root_channel);
...
// ============================================================================
// M2b-4 — in-circuit CROSS-TABLE CHANNEL JOIN over B512.
// ============================================================================
...
impl ChildTableB512 {
	pub fn new(cs: &mut ConstraintSystem<OurB512>, join: ChannelId) -> Self {
		let mut table = cs.add_table("M2b-4 child SHA3-256 over B512 (push root to join)");
...
impl ParentTableB512 {
	pub fn new(
		cs: &mut ConstraintSystem<OurB512>,
		join: ChannelId,
		root_channel: Option<ChannelId>,
	) -> Self {
		let mut table = cs.add_table("M2b-4 parent SHA3-256 over B512 (pull inner root from join)");
...
/// Two-table, one-channel child->parent join over B512.
pub fn prove_verify_join_b512(
	a: [u8; 32],
	b: [u8; 32],
	d: [u8; 32],
	mode: JoinMode,
	...
	let child = ChildTableB512::new(&mut cs, join);
	let parent = ParentTableB512::new(&mut cs, join, root_channel);

	let mut boundaries: Vec<Boundary<OurB512>> = vec![];
...
mod tests {
	...
	// ---------------------------- M2b-2 seam over B512 -----------------------

	#[test]
	fn seam_over_b512_honest_accepts() {
		let a = [0xAAu8; 32];
		let b = [0xBBu8; 32];
		let c = [0xCCu8; 32];
		...
		let (proof_size, g1_digests, roots) =
			prove_verify_seam_b512(&children, 1, 256).expect("honest seam over B512 must verify");

		assert_eq!(g1_digests[0], d1_exp, "in-circuit g1 digest over B512 != native");
		assert_eq!(roots[0], root_exp, "in-circuit root over B512 != native");
		assert!(proof_size > 0);
		println!(
			"M2b-2/B512 seam honest ACCEPTS at L5(256): depth-2 chain verified over the 512-bit field; \
			 g1 digest & root match native; proof = {proof_size} bytes"
		);
	}

	#[test]
	fn seam_over_b512_forged_link_rejected() {
		let forged = [0xDEu8; 32];
		let report =
			forged_link_rejected_b512(forged, 1, 256).expect("adversarial harness must run over B512");

		assert!(report.validate_rejected, "SOUNDNESS FAILURE: validate accepted forged link over B512");
		assert!(report.pipeline_rejected, "SOUNDNESS FAILURE: pipeline accepted forged link over B512");
		assert!(
		...
		println!(
			"M2b-2/B512 forged-link REJECTED (isolated to seam) at L5(256):\n  \
			 firing constraint: {}\n  pipeline stage   : {}\n  pipeline error   : {}",
			report.validate_error, report.pipeline_stage, report.pipeline_error
		);
	}

	// ------------------------- M2b-3 root boundary over B512 -----------------

	#[test]
	fn root_boundary_over_b512_correct_accepts() {
		let a = [0xAAu8; 32];
		let b = [0xBBu8; 32];
		let c = [0xCCu8; 32];
		...
		let children = vec![((a, b), c)];
		let outcome = prove_verify_seam_root_boundary_b512(&children, root_exp, 1, 256)
			.expect("root-boundary harness must run over B512");

		assert_eq!(outcome.root, root_exp, "in-circuit root over B512 != native");
		assert!(
			outcome.accepted(),
			"correct root over B512 rejected at '{}': validate='{}' prove='{}' verify='{}'",
		...
		println!(
			"M2b-3/B512 correct-root ACCEPTS at L5(256): root enforced as public boundary (top field = B512); proof = {} bytes",
			outcome.proof_size
		);
	}

	#[test]
	fn root_boundary_over_b512_wrong_rejected() {
		let a = [0xAAu8; 32];
		let b = [0xBBu8; 32];
		let c = [0xCCu8; 32];
		let (_d1, root_exp) = native_chain(&a, &b, &c);
		...
		let children = vec![((a, b), c)];
		let outcome = prove_verify_seam_root_boundary_b512(&children, wrong, 1, 256)
			.expect("root-boundary harness must run over B512");

		assert_eq!(outcome.root, root_exp);
		assert!(
			!outcome.accepted(),
			"SOUNDNESS FAILURE: a WRONG claimed root was ACCEPTED over B512"
		);
		assert!(
			!outcome.verify_ok,
			"SOUNDNESS FAILURE: SHA-256 verify over B512 accepted a wrong claimed root"
		);
		println!(
			"M2b-3/B512 wrong-root REJECTED at stage '{}':\n  validate: ok={} err='{}'\n  verify  : ok={} err='{}'",
			outcome.reject_stage,
			outcome.validate_ok,
			outcome.validate_error,
			outcome.verify_ok,
	...
	// ---------------------------- M2b-4 join over B512 -----------------------

	#[test]
	fn join_over_b512_honest_accepts() {
		let a = [0xAAu8; 32];
		let b = [0xBBu8; 32];
		let d = [0xDDu8; 32];
		let (rc_exp, rp_exp) = native_join(&a, &b, &d);

		let outcome = prove_verify_join_b512(a, b, d, JoinMode::Honest, None, 1, 256)
			.expect("honest join harness must run over B512");

		assert_eq!(outcome.r_child, rc_exp, "in-circuit R_child over B512 != native");
		assert_eq!(outcome.r_parent, rp_exp, "in-circuit R_parent over B512 != native");
		assert!(
			outcome.accepted(),
			"honest join over B512 rejected at '{}': validate='{}' prove='{}' verify='{}'",
			outcome.reject_stage,
			outcome.validate_error,
			outcome.prove_error,
			outcome.verify_error
		...
		assert!(outcome.proof_size > 0);
		println!(
			"M2b-4/B512 join honest ACCEPTS at L5(256): child->parent channel balanced over the 512-bit field; \
	...
	#[test]
	fn join_over_b512_forged_inner_root_rejected() {
		let a = [0xAAu8; 32];
		let b = [0xBBu8; 32];
		let d = [0xDDu8; 32];
		...
		let outcome =
			prove_verify_join_b512(a, b, d, JoinMode::ForgedInnerRoot { forged }, None, 1, 256)
				.expect("forged-join harness must run over B512");

		assert_eq!(outcome.r_child, rc_exp);
		assert!(
			!outcome.accepted(),
			"SOUNDNESS FAILURE: a FORGED inner root was ACCEPTED over B512"
		);
		assert!(
			!outcome.verify_ok,
			"SOUNDNESS FAILURE: SHA-256 verify over B512 accepted a forged inner root"
		);
		println!(
			"M2b-4/B512 forged-inner-root REJECTED at stage '{}':\n  validate: ok={} err='{}'\n  verify  : ok={} err='{}'",
			outcome.reject_stage,
			outcome.validate_ok,
			outcome.validate_error,
			outcome.verify_ok,
