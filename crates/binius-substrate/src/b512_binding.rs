...
// the load-bearing IN-CIRCUIT soundness primitive proves AND verifies at NIST L5
// Fiat–Shamir security (security_bits = 256).
//
// This mirrors `b256_binding.rs` exactly, ONE tower level up (B256 -> B512). It is
// `sha3_binding.rs` (M2b-1, over the DEFAULT CanonicalTowerFamily / B128) re-
// threaded through the B512 prove wiring proven out in `b512_keccak.rs` /
// `b512_sha3.rs`:
//   * `ConstraintSystem::<B512>` instead of `ConstraintSystem<B256>` / the default
//     B128.
//   * `WitnessIndex::<B512>` (B512 is its own width-1 packed field), so the witness
//     segment is `TableWitnessSegment<B512>`.
//   * `prove/verify::<U512, B512TowerFamily, Sha256, Sha256Compression,
//     HasherChallenger<Sha256>>` — the 512-bit challenge field makes the FRI /
//     sumcheck error terms poly(N)/2^512 clear the NIST L5(256) query budget, the
//     exact level where BOTH B128 and B256 fail out of `calculate_n_test_queries`
//     (that is the whole point of B512).
//
// ============================ WHAT IS SOUND (unchanged from M2b-1) ===========
// The binding zero-constraints on the *committed* `state_in` columns of the
// wrapped `Keccakf` gadget are IDENTICAL to the B128 / B256 versions — they operate
// purely on B1 columns via a track-0 mask and `add_constant` transparent columns,
// both of which are field-generic (the fork's Phase B made `Keccakf` / `add_constant`
...
// missing closing bit, …) FAILS the zerocheck — now over the 512-bit field.
//
...
// `TableWitnessSegment::get_mut_as`) plus the B512 tower family. `assert_zero` over
// `ConstraintSystem<B512>` and `add_constant` over B512 both compile+prove unchanged
// — no residual B128/B256 hard-coding in either op.
// ============================================================================
...
/// Witness segment packed type: B512 as its own width-1 packed field (matches
/// `WitnessIndex::<B512>` in `b512_keccak.rs`).
...
/// A single-table constraint system OVER B512 that proves a batch of single-block
/// SHA3-256 hashes AND binds each `state_in` to the FIPS-202 padding of a committed
/// 3-byte message.
pub struct Sha3BindingTableB512 {
...
impl Sha3BindingTableB512 {
	pub fn new(cs: &mut ConstraintSystem<OurB512>) -> Self {
		let mut table = cs.add_table("SHA3-256 sound single block over B512 (M2b-1)");
		...
		// is generic over the top field (Phase B); exercised here over B512.
	...
	/// column over the B512 witness segment.
...
/// `Col<B1,512>` witness buffer over the B512 segment, viewed as 8 u64 tracks/row.
...
/// Honest end-to-end OVER B512: prove + verify sound single-block SHA3-256 for each
/// 3-byte message, under a SHA-256 (FIPS 180-4) Merkle commitment and SHA-256
/// Fiat–Shamir transcript, with the challenge/extension field = `B512` (2^512).
/// Returns `(proof_size_bytes, in_circuit_digests)`.
pub fn prove_verify_bound_sha3_b512(
	messages: &[Vec<u8>],
	log_inv_rate: usize,
	security_bits: usize,
	...
	let table = Sha3BindingTableB512::new(&mut cs);
	...
	binius_core::constraint_system::validate::validate_witness(&ccs, &[], &witness)
		.map_err(|e| anyhow::anyhow!("honest witness failed validate_witness over B512: {e}"))?;
...
/// Adversarial soundness gate OVER B512: build a single-message witness whose
...
/// SHA-256 prove/verify pipeline with the 512-bit challenge field.
pub fn bad_witness_rejected_b512(
	corrupt: Corruption,
	log_inv_rate: usize,
	security_bits: usize,
) -> Result<RejectReport> {
	...
	// 2) End-to-end SHA-256 pipeline over B512: prove then verify.
	let proof = binius_core::constraint_system::prove::<
		U512,
		B512TowerFamily,
		Sha256,
...
mod tests {
	...
	#[test]
	fn binding_state_matches_m2a_padding_b512() {
	...
	/// "abc" proves and verifies OVER B512 at NIST L5 (security_bits=256), and the
	/// in-circuit digest equals the NIST SHA3-256("abc") vector. THIS is the load-
	/// bearing soundness primitive at L5.
	#[test]
	fn bound_sha3_over_b512_accepts() {
		let messages = vec![b"abc".to_vec()];
		let (proof_size, digests) = prove_verify_bound_sha3_b512(&messages, 1, 256)
			.expect("honest bound SHA3-256 proof must VERIFY over B512 at NIST L5 (256)");

		assert_eq!(digests.len(), 1);
		assert_eq!(digests[0], NIST_ABC, "in-circuit bound SHA3-256(\"abc\") over B512 != NIST vector");
		assert_eq!(digests[0], native_sha3_256(b"abc"), "over B512 != sha3 crate");
		assert!(proof_size > 0);
		println!(
			"M2b-1/B512 ACCEPTS: sound SHA3-256(\"abc\") message-bound + padding-pinned, VERIFIED \
			 at NIST L5(256) over the 512-bit challenge field; digest == NIST + sha3; proof = {proof_size} bytes"
		);
	}
	...
	/// Keccak-f (in, out) permutation pair — MUST be rejected OVER B512, isolated to
	/// the new padding binding.
	#[test]
	fn bound_sha3_over_b512_bad_padding_rejected() {
		let report = bad_witness_rejected_b512(Corruption::DomainByte, 1, 256)
			.expect("adversarial harness must run to completion over B512");

		assert!(
			report.validate_rejected,
			"SOUNDNESS FAILURE: validate_witness accepted a bad-padding state_in over B512"
		);
		assert!(
			report.pipeline_rejected,
			"SOUNDNESS FAILURE: SHA-256 prove/verify over B512 accepted a bad-padding state_in"
		);
		assert!(
		...
		println!(
			"M2b-1/B512 bad-padding (domain 0x06->0x00) REJECTED at NIST L5(256):\n  \
			 firing constraint: {}\n  pipeline stage   : {}\n  pipeline error   : {}",
	...
	/// (non-sponge input) must also be rejected by the capacity binding over B512.
	#[test]
	fn bound_sha3_over_b512_bad_capacity_rejected() {
		let report = bad_witness_rejected_b512(Corruption::CapacityLane, 1, 256)
			.expect("adversarial harness must run to completion over B512");

		assert!(
			report.validate_rejected,
			"SOUNDNESS FAILURE: validate_witness accepted a nonzero-capacity state_in over B512"
		);
		assert!(
			report.pipeline_rejected,
			"SOUNDNESS FAILURE: SHA-256 prove/verify over B512 accepted a nonzero-capacity state_in"
		);
		assert!(
		...
		println!(
			"M2b-1/B512 bad-capacity (lane17 != 0) REJECTED at NIST L5(256):\n  \
			 firing constraint: {}\n  pipeline stage   : {}",
