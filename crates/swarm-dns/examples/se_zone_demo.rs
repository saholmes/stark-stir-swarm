...
async fn main() {
    ...
    println!("      Mac M-series at N=16, Ni=4, K=4.  Soundness column shows which");
    println!("      stages clear L1 (128 bits unconditional Johnson, ½·log₂(blowup)/q):");
    println!("        config                 inner-prove shard-prove verify   L1 wire  sound");
    println!("        ─────────────────────  ─────────── ─────────── ───────  ───────  ─────");
    println!("        FAST-ITER (4/4/4 r=135/135/54)  56.0 s   13.4 s  9.27 ms 2491 KiB  M:✓ R:✗ I:✗");
    println!("        MIXED     (4/32/32 r=54/54/54)  56.7 s   42.2 s  5.50 ms 1504 KiB  M:✓ R:✓ I:✗");
    println!("        PROD-L1   (32/32/32 r=54/54/54) 386.9 s  42.2 s  5.54 ms 1504 KiB  M:✓ R:✓ I:✓");
    println!("      Legend: M=master, R=top-Merkle, I=inner-v2.");
    println!("      Only PROD-L1 has every stage at L1.  FAST-ITER and MIXED have inner-v2");
    println!("      at only 54 bits (V2_NUM_QUERIES=54 hardcoded, gives ½·log₂(4)·54=54 bits");
    println!("      at blowup=4 — sub-L1).  Set INNER_BLOWUP=32 to recover full soundness");
    println!("      on the inner stage (the +6.8× prove cost relative to inner=4).");
    println!("      Conclusion on edge-consumer wins (independent of inner): master/merkle=32");
    println!("      gives 40 %% smaller wire + 41 %% faster verify vs FAST-ITER, at 3.1× shard");
    println!("      prove cost.  Wire stays constant at fixed K as N grows; doubling K adds");
    println!("      ~130 KiB to the super-master.");
