...
DNSSEC lets a resolver authenticate DNS records, but its classical signatures
(RSA, ECDSA, Ed25519) are broken by a cryptographically relevant quantum
computer. A transparent, post-quantum alternative is to prove, in zero
network round trips, that a zone's records were correctly signed under the
published keys: publish a succinct \emph{epoch} proof once, and let any resolver
verify it locally. Such ``epoch DNS-STARK'' constructions have been proposed;
the engineering question this paper addresses is how to \emph{aggregate} the many
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  FIPS-approved function (SHA-2 or SHA-3). Arithmetization-friendly hashes
  (Poseidon, Rescue, Vision) are excluded.
...
\paragraph{Binary-tower STARKs.} We work over a tower of binary fields
...
FRI-Binius: a ring-switch reduction turns a committed-multilinear opening into a
sumcheck, and the sumcheck folds \emph{in lockstep} with a FRI proximity test,
sharing one challenge per round. The vector commitment is a Merkle tree over a
...
the Fiat--Shamir resistance. For NIST category-$N$ security every term must reach
$N$; a large field with a 128-bit hash is pinned at 128 bits.
...
Accumulation attacks Observation~\ref{obs:tension} at its root: it never verifies
the inner proof in-circuit. Each step folds a new instance into a constant-size
accumulator with a cheap operation; a single decider checks the accumulator at
the end. The recursion step arithmetizes the \emph{fold}, not the verifier.
...
folding forms $\sum_i \lambda_i\,\mathrm{Com}(P_i)$ and needs an additively
homomorphic commitment; a Merkle-over-hash commitment has none. We sidestep this
...
Accumulation (Nova, ProtoStar, Protogalaxy) achieves millisecond IVC by folding
instances with homomorphic commitments; Binius's hash commitments preclude the
direct construction, but Observation~\ref{obs:native} shows the same end is
reached natively. Arithmetization-friendly hashes (Poseidon/Vision) narrow the
recursion circuit but are non-FIPS. Holographic SNARKs make the verifier succinct
in width but Binius is not holographic. Our route keeps the FIPS hash, keeps the
...
\paragraph{Relation to query-reduction.} STIR and related low-degree tests cut
FRI query complexity. Over Binius this reduces proof size and the number of
in-circuit openings, but not the width-linear verify cost
(Observation~\ref{obs:width}); it is orthogonal to the tension resolved here.
