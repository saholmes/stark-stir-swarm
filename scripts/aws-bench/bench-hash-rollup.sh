#!/usr/bin/env bash
# Hash-rollup STARK at trace sizes 2^16, 2^18, 2^20 (paper-aligned).
# 2^20 OOM-thrashes c5.4xlarge but fits on r8g.4xlarge / r5.4xlarge
# (128 GiB).  Override `BENCH_HASH_ROLLUP_K` to "16 18" on
...
K_RANGE="${BENCH_HASH_ROLLUP_K:-16 18 20}"
